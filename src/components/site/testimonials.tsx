...
const categoryConfig = {
  live: { label: "Live", icon: Home, color: "text-[#1F7A3A] border-[#1F7A3A]/30 bg-[#1F7A3A]/5" },
  ...
  diaspora: { label: "Diaspora", icon: Plane, color: "text-[#1F7A3A] border-[#1F7A3A]/30 bg-[#1F7A3A]/5" },
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10">
        <ScrollReveal className="mb-12 text-center">
          ...
          <h2 className="mt-4 font-serif text-[clamp(36px,4.4vw,60px)] font-normal leading-[1.12] text-[#102357]">
            Ways to <em className="italic text-[#1F7A3A]">Own & Earn</em>
          </h2>
          <p className="mx-auto mt-4 max-w-[640px] text-[15px] font-light text-[#6b7280]">
            Explore possible ways to use a residence. Pricing, income and terms
            are illustrations only; request the latest documents before deciding.
          </p>
        </ScrollReveal>
                ...
                return (
                  <CarouselItem key={t.id} className="basis-full pl-4 sm:basis-1/2 lg:basis-1/3">
                    <div className="flex h-full flex-col bg-white p-7 shadow-[0_8px_24px_rgba(8,21,52,0.06)]">
                      ...
                      <div className="mt-1 text-[11px] font-semibold uppercase tracking-wider text-[#1F7A3A]">
                        {t.scenario}
                      </div>
        ...
        <ScrollReveal delay={150} className="mt-10 text-center">
          <p className="text-[12px] text-[#6b7280]">
            ...
            Illustrative scenarios are not client testimonials, offers or
